...
<template>
  <span class="inline-flex items-center gap-1.5 text-xs font-semibold tracking-wide uppercase">
    <span class="h-2 w-2 rotate-45" :style="tierSwatchStyle(tierInfo.colorVar, tierInfo.gradient)" />
    <span :style="tierTextStyle(tierInfo.colorVar, tierInfo.gradient)">{{ tierInfo.label }}</span>
